...
```js
const newComment = new Comment({ post: postId, user: userId, content: 'Nice post!' });
...
### **Trả lời bình luận**
```js
const replyComment = new Comment({ post: postId, user: userId, content: 'Replying...', replies: [commentId] });
await replyComment.save();
await Comment.updateOne({ _id: commentId }, { $push: { replies: replyComment._id } });
```
### **Lấy danh sách trả lời bình luận**
```js
const commentWithReplies = await Comment.findById(commentId).populate('replies');
